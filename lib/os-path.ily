...
#(define-public (oll:os-path-split path)
...
% All the functions take a 'path' argument as processed by oll:os-path-split.

#(define-public (oll:os-path-join path)
   "Converts a given path to a path corresponding to the OS convention"
   (string-join (oll:os-path-split path) os-path-separator-string))

#(define-public (oll:os-path-join-unix path)
   "Converts a given path to a unix-like path"
   (string-join (oll:os-path-split path) "/"))

#(define-public (oll:os-path-join-dots path)
...
#(define-public (oll:os-path-absolute path)
   ...
   (let* ((path-list (oll:os-path-normalize path)))
     (if (oll:oll:os-path-absolute? path-list)
         path-list
         (append
          (oll:os-path-cwd-list)
          path-list))))

#(define-public (oll:os-path-normalize path)
        ...
        (set! result
              (cond
               ...
               ((equal? e ".")
                result)
               (else `(,e ,@result)))))
      (oll:os-path-split path))
     (reverse result)))


#(define-public (oll:os-path-cwd-list)
   "Return the current working directory as a list of strings."
   (oll:os-path-split (getcwd)))

#(define-public (oll:os-path-dirname path)
...
#(define-public (oll:location->normalized-path location)
   "Returns a normalized path to the given location object"
   (oll:os-path-normalize (car (ly:input-file-line-char-column location))))

#(define-public (oll:location-extract-path location)
   "Returns the normalized path from a LilyPond location
    or './' if 'location' is in the same directory."
   (let* ((loc (oll:location->normalized-path location))
...
#(define-public (oll:this-file) (oll:location->normalized-path (*location*)))

% Return the normalized absolute path of the directory containing "this"
#(define-public (oll:this-dir)
   (let ((file (oll:this-file)))
     (list-head file (- (length file) 1))))

% Return the parent of (oll:this-dir)
...
#(define-scheme-function ()()
   ...
   (let ((outname (ly:parser-output-name (*parser*)))
         (locname (oll:location->normalized-path (*location*))))
     (ly:message outname)
